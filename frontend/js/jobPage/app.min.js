function individualPage() {
  ...
  function map(jobData, locations, cities) {
    ...
    function getGeoData() {
      cities.forEach((loc, i) => {
        const coordinates = { lng: loc.Longitude, lat: loc.Latitude };
    ...
    function addMapPoints() {
      ...
      mapLocations.features.forEach((location) => {
        ...
        cityEl.addEventListener('click', () => {
          map.flyTo({ center: coordinates, speed: 1, curve: 1, zoom: 13 });
          clicked = !0;
        ...
        function innerLocShow() {
          locations.forEach((loc) => {
            if (!loc) return;
            const coords = { lng: loc.Logtitude, lat: loc.Latitude };
    ...
    filter_link.remove();
    const cloneAllButn1 = filter_link.cloneNode(!0);
    const cloneAllButn2 = filter_link.cloneNode(!0);
    const cloneAllButn3 = filter_link.cloneNode(!0);
    ...
    sortedCities.forEach((item) => {
      const clone1 = filter_link.cloneNode(!0);
    ...
    jobCategories.forEach((item) => {
      const clone2 = filter_link.cloneNode(!0);
    ...
    teamCategories.forEach((item) => {
      const clone = filter_link.cloneNode(!0);
    ...
    function filteringSystem() {
      ...
      let activeFilters = { location: [], category: [], teamCategory: [] };
  ...
  function latestJobs(allJobs) {
    ...
    locationCategories.forEach((cat) => {
      const clone = filter_li.cloneNode(!0);
    ...
    allJobs.forEach((job) => {
      const card = searchTemplate.content.cloneNode(!0).children[0];
    ...
    function filterSystem() {
      ...
      let currentPage = 1;
      let itemsPerPage = 6;
      let activeFilters = { location: [], team: [], contract: [] };
